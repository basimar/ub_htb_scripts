...
DSV05_DIR=/home/htb/alephdata
DSV05_SEQ=$DSV05_DIR/dsv05.seq
HOME=/home/htb/han_gosteli_archiv
...
../htb/htb_alephseq_to_marcxml tmp/gosteli-all.seq tmp/gosteli-all.xml

say "* extrahiere 490er"
../htb/htb_alephseq_extract_490 tmp/gosteli-all.seq tmp/gosteli-490.seq

say "* extrahiere Toplevel Info (Personen)"
saxon9 -versionmsg:off -xsl:../htb/extract-top-info.xsl -s:tmp/gosteli-all.xml -o:tmp/p-top.txt typ=p
...
saxon9 -versionmsg:off -xsl:../htb/extract-top-info.xsl -s:tmp/gosteli-all.xml -o:tmp/v-top.txt typ=k
...
perl ../htb/htb_build_hierarchy_top_down --toplist=tmp/p-top.txt --list490=tmp/gosteli-490.seq --outfile=tmp/p-hierarchy.xml

say "* generiere top-down Hierarchie (Verbände)"
perl ../htb/htb_build_hierarchy_top_down --toplist=tmp/v-top.txt --list490=tmp/gosteli-490.seq --outfile=tmp/v-hierarchy.xml

say "* generiere top-down Hierarchie (Biographische Notizen)"
perl ../htb/htb_build_hierarchy_top_down --toplist=b-top.txt --list490=tmp/gosteli-490.seq --outfile=tmp/b-hierarchy.xml

say "* reichere Hierarchie an (Personen)"
perl ../htb/enrich-hierarchy-with-marcdata-simple.pl --infile=tmp/p-hierarchy.xml --marcfile=tmp/gosteli-all.xml --outfile=tmp/p-hierarchy-full.xml --author=N


say "* reichere Hierarchie an (Verbände)"
perl ../htb/enrich-hierarchy-with-marcdata-simple.pl --infile=tmp/v-hierarchy.xml --marcfile=tmp/gosteli-all.xml --outfile=tmp/v-hierarchy-full.xml --author=N

say "* reichere Hierarchie an (Biografische Notizen)"
perl ../htb/enrich-hierarchy-with-marcdata-simple.pl --infile=tmp/b-hierarchy.xml --marcfile=tmp/gosteli-all.xml --outfile=tmp/b-hierarchy-full.xml --author=N
...
saxon9 -versionmsg:off -xsl:../htb/tigra_tree_menu_full.xsl \
